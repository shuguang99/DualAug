...
from Augment.common import apply_weightnorm
from Augment.networks.convnet import SeqConvNet
from Augment.networks.mlp import MLP
from Augment.networks.resnet import ResNet
from Augment.networks.shakeshake.shake_resnet import ShakeResNet
from Augment.networks.shakeshake.shake_resnext import ShakeResNeXt
from Augment.networks.wideresnet import WideResNet
from Augment.networks.wideresnet import WideResNetCosine
from Augment.networks.models.inceptionv4 import InceptionV4
...
def get_model(conf, bs, num_class=10, writer=None):
    ...
    if name == 'resnet50':
        ...
        model = ResNet(dataset='imagenet', depth=18, num_classes=num_class, bottleneck=True)
    elif name == 'resnet200':
        model = ResNet(dataset='imagenet', depth=200, num_classes=num_class, bottleneck=True)
    elif name == 'wresnet40_2':
        model = WideResNet(40, 2, dropout_rate=conf.get18('dropout',0.0), num_classes=num_class, adaptive_dropouter_creator=ad_creators[0],adaptive_conv_dropouter_creator=ad_creators[1], groupnorm=conf.get('groupnorm', False), examplewise_bn=conf.get('examplewise_bn', False), virtual_bn=conf.get('virtual_bn', False))
        ...
        model = inception_v4(num_classes=num_class)
    else:
